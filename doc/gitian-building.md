...
*Setup instructions for a gitian build of Tecax using a Debian VM or physical system.*
...
Clone the git repositories for tecax and gitian and then checkout the tecax version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the tecax repository
...
To build Tecax (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the tecax repository.
...
```bash
    ...
    From https://github.com/tecax/tecax
...
For example:
```bash
URL=https://github.com/konez2k/tecax.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tecax-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tecax-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/tecax-build.assert
```
...
[tecax/gitian.sigs](https://github.com/tecax/gitian.sigs/) repository, or if that's not possible to create a pull
request.
